...
async function review_adder() {
    try {
        ...
        for (let i = 0; i < result.rows.length; i++) {
            ...
            var variables = {
                id: result.rows[i].ANIME_ID // Use the actual ANIME_ID from the result
            };
            ...
            console.log(result.rows[i].ANIME_ID);
            ...
            for(let j=0; j<reviews.length; j++){
                try{
                    const result2 = await connection.execute(
                        `INSERT INTO USER_ANIME_REVIEWS(REVIEW_ID, ANIME_ID, USER_ID, SCORE, VOTES, SUMMARY, REVIEW)
                        VALUES (:review_id, :anime_id, :user_id, :score, :votes, :summary, :review)`,
                        [
                            reviews[j].node.id,
                            result.rows[i].ANIME_ID,
                            getRandomInt(1, 3009),
